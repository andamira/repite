// repite::looper
//
//! A loop manager with support for multiple rates.
//
...
use crate::{error::Result, Rate, RateStats};
...
impl Looper {
    ...
    /// # Examples
    /// ```
    /// use repite::{Looper, Rate};
    ///
    /// let mut l = Looper::new();
    /// assert![l.add_rate("ascii_name_max_length", Rate::with_tps(60.), false).is_ok()];
    /// ```
    #[inline(always)]
    pub fn add_rate(&mut self, name: &str, rate: Rate, stats: bool) -> Result<Option<Rate>> {
